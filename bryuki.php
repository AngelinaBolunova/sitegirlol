...
if ($colorFilter) {
    $stmt = $conn->prepare("SELECT id, name, price, image_path, description, quantity FROM bryuki WHERE color = ?");
    $stmt->bind_param("s", $colorFilter);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $query = "SELECT id, name, price, image_path, description, quantity FROM bryuki";
...
<head>
    ...
    <title>Отдел Брюки - Девичья феерия</title>
...
<body>
    ...
    <div class="search-bar">
        <form method="get" action="bryuki.php" id="filterForm" style="display: inline-block;">
            ...
            <select name="color" id="colorFilter" onchange="document.getElementById('filterForm').submit();">
                ...
                <option value="желтый" <?php if (isset($_GET['color']) && $_GET['color'] === 'желтый') echo 'selected'; ?>>Желтый</option>
    ...
    <main>
        <nav style="margin-bottom: 20px; text-align: center;">
            ...
            <a href="bryuki.php" style="margin: 0 10px; color: #d81b60; font-weight: bold; text-decoration: underline;">Брюки</a>
        </nav>
        <h1 class="category-header">Отдел Брюки</h1>
            ...
            <?php foreach ($products as $product): ?>
                <div class="product">
                    ...
                    <form method="post" action="cart.php">
                        ...
                        <input type="hidden" name="category" value="bryuki" />
