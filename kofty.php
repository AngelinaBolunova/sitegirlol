...
if ($colorFilter) {
    $stmt = $conn->prepare("SELECT id, name, price, image_path, description, quantity FROM kofty WHERE color = ?");
    $stmt->bind_param("s", $colorFilter);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $query = "SELECT id, name, price, image_path, description, quantity FROM kofty";
...
<head>
    ...
    <title>Отдел Кофты - Девичья феерия</title>
...
<body>
    ...
    <div class="search-bar">
        <form method="get" action="kofty.php" id="filterForm" style="display: inline-block;">
            ...
            <select name="color" id="colorFilter" onchange="document.getElementById('filterForm').submit();">
                ...
                <option value="голубой" <?php if (isset($_GET['color']) && $_GET['color'] === 'голубой') echo 'selected'; ?>>Голубой</option>
                <option value="бордовый" <?php if (isset($_GET['color']) && $_GET['color'] === 'бордовый') echo 'selected'; ?>>Бордовый</option>
    ...
    <main>
        <nav style="margin-bottom: 20px; text-align: center;">
            <a href="futbolki.php" style="margin: 0 10px; color: #d81b60; font-weight: bold; text-decoration: none;">Футболки</a> |
            <a href="kofty.php" style="margin: 0 10px; color: #d81b60; font-weight: bold; text-decoration: underline;">Кофты</a> |
            <a href="bryuki.php" style="margin: 0 10px; color: #d81b60; font-weight: bold; text-decoration: none;">Брюки</a>
        </nav>
        <h1 class="category-header">Отдел Кофты</h1>
            ...
            <?php foreach ($products as $product): ?>
                <div class="product">
                    ...
                    <div class="product-quantity" style="font-weight: 600; color: #880e4f; margin-bottom: 10px;">
                        Количество: <?php echo intval($product['quantity']); ?>
                    </div>
                    <form method="post" action="cart.php">
                        ...
                        <input type="hidden" name="category" value="kofty" />
